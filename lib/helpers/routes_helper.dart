...
abstract class RoutesHelper {
  ...
  static Future<dynamic> toMessages({
    ...
    // Create initial chat for individual conversations
    if (!isGroup && user != null) {
      final User currentUser = AuthController.instance.currentUser;
      
      // Create an empty chat entry to show in the chat list
      final Message emptyMessage = Message(
        msgId: AppHelper.generateID,
        senderId: currentUser.userId,
        type: MessageType.text,
        textMsg: '', // Empty message
      );
